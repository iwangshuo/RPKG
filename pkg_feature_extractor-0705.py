...
class FeatureExtractor:
    def __init__(self):
        ...
        self.pkg_name_feature_path = os.path.join(cur_dir, 'output/pkg_name_feature-20230705.csv')
        self.pkg_name_other_feature_path = os.path.join(cur_dir, 'output/pkg_name_other_feature-20230705.csv')
        self.feature_entity_dict_path = os.path.join(cur_dir, 'output/feature_entity_dict-20230705.csv')
        # 领域名词（机器人名字、传感器名字等）
    ...
    def extract_pkg_name(self):
        ...
        f4 = open(self.feature_entity_dict_path, "w", newline="")
        dict_writer = csv.writer(f4) # delimiter='，'
        ...
        with open(self.pkg_name_feature_path, "w", newline="") as f3:
            ...
            for pkg_name in column2:
                ...
                if robot_name in self.feature_entity_dict.keys():
                    ...
                    single_row = [pkg_name, robot_name, self.feature_entity_dict[robot_name][0], self.feature_entity_dict[robot_name][1]]
                    ...
                    if match_region[1] >= 89:
                        ...
                        if robot_name == "turtlebot":
                            ...
                            if fuzzy_match_results[1][1] >= 89:
                                ...
                                non_region_words.remove(robot_name)
                                writer.writerow(single_row)
                                self.feature_entity_dict[robot_name] = [fuzzy_match_results[1][0], fuzzy_match_results[1][1]]
                        elif robot_name not in self.robot_common_wds:
                            non_region_words.remove(robot_name)
                            writer.writerow(single_row)
                            self.feature_entity_dict[robot_name] = [match_region[0], match_region[1]]
                        else:
                            pass
        ...
        for key, value in self.feature_entity_dict.items():
            dict_writer.writerow([key, value[0], value[0]])
